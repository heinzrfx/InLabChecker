...
- **Pegadinhas de `RayMeshGridIntersect` e `Mesh_Select` por script** (achadas na sondagem do passo 3 da Otimização, ver spec `2026-09-23-otimizacao-design.md` seção 5 e o cabeçalho de `fn_otim_escondidas.ms`):
  - Com vários nós numa grade, os raios erram as peças longe do primeiro `addNode` (a grade parece dimensionada por ele). Use uma grade por peça.
  - `intersectRay` conta **triângulos** atingidos: raio pela diagonal de um quad conta 2. Deduplique pela distância e use direções não alinhadas a eixo. Com `doubleSided:false` só conta face de frente para o raio: para paridade (dentro/fora) use `true`.
  - `intersectSphere` devolveu 0 em todos os raios e posições testados: não use. Distância até a malha: `closestFace p` e depois `getHitDist 1`.
  - A grade é um retrato do momento do `buildGrid`, em coordenadas de mundo: mover o nó depois não atualiza. A resolução do `Initialize` pesa muito no tempo (fixa 10 = 68 s, 80 = 21 s na mesma cena de 105 mil triângulos).
  - `getHitNormal` devolve a normal da face atingida **invertida** (Max 2024). Para a normal de verdade, guarde o `snapshotAsMesh` do nó no momento do `buildGrid` e use `getFaceNormal malha (getHitFace i)`. `intersectRay` às vezes devolve acertos com distância `inf.0` (raio por vértice comum a vários triângulos): descarte antes de contar paridade.
  - Paridade (dentro/fora) só vale contra malha fechada: plano ou casca aberta inclinada dá acerto ímpar em várias direções. Confira `(meshop.getOpenEdges malha).numberSet == 0`.
  - `-bitarray` complementa só até o maior índice marcado, não até o total de faces: use `#{1..n} - marcadas`.
  - `setFaceSelection <nó> <Mesh_Select> faces` só vale com o nó selecionado, painel Modify, o `Mesh_Select` corrente e `subObjectLevel = 3`. Um `DeleteMesh` posto depois com `addModifier` apaga a peça inteira (o nível volta a objeto); com `modPanel.addModToSelection` apaga só a seleção.
  - Desenho de `registerRedrawViewsCallback` com `gw.*` respeita o z-buffer: para mostrar algo escondido atrás de outra peça, `gw.setRndLimits #(#wireframe)` (sem `#zBuffer`).
