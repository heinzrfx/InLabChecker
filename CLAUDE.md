...
- **Spline renderizável** (Line com `render_displayRenderMesh`/`render_renderable`, sem modificador) é `Shape`, não `GeometryClass`: `InLab_EhGeometria` e `InLab_Otim_Triangulos` não a contam, e `GetTriMeshFaceCount` devolve 0 até o nó ser avaliado (`classof o`). Para medir, avalie antes ou use `snapshotAsMesh` (ver `InLab_OtimOrigem_TrisNo` em `fn_otim_origem.ms`). Propriedade de objeto mudada por script (ex.: `sides`) é refletida por `GetTriMeshFaceCount` na hora; para medir custo sem sujar o undo, `with undo off` e volte o valor original mesmo com exceção.
